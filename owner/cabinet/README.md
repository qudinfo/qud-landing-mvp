# Stage 9 UI refinement — Compact Card & Adaptive Sidebar, 11.09.2026

Desktop sidebar работает как компактная навигационная rail-панель шириной
92 px и раскрывается до 310 px поверх рабочей области по наведению или при
клавиатурном фокусе. Рабочая область не сдвигается во время раскрытия. На
экранах до 820 px сохраняется исходное выезжающее hamburger-меню.

Strategy Card собрана в компактный двухколоночный dashboard: идентичность и
восемь KPI находятся слева, график — справа. Подтверждённые периоды свёрнуты
по умолчанию и раскрываются по запросу пользователя. На мобильных KPI остаются
в сетке 2×4, график показывает три основные подписи дат, а данные и API-flow
Stage 9 остаются неизменными.

---

# Этап 9 — Strategy Card & History Graph, 11.09.2026

Строки реального каталога теперь открывают детальную карточку через
`GET /owner/api/strategies/{strategy_id}`. Список и уже загруженные карточки
кешируются только в памяти открытой страницы; возврат «Все стратегии» не
повторяет запрос списка, а показ формы входа полностью очищает оба кеша.

Карточка показывает Strategy ID, Trust Score, Trust Zone, восемь согласованных
KPI, график накопительного результата по всем подтверждённым периодам и
компактную таблицу истории. График использует `period_end` по оси X и только
`strategy_return_since_start_pct` по оси Y; значения капитала, balance, equity
и данные Virtual Portfolio не используются. Для отсутствующих Max Drawdown и
SL Safe выводится «Недостаточно подтверждённых данных».

Реализованы отдельные состояния загрузки, ошибки с повтором, отсутствующей
стратегии и потери сессии через существующий Stage 7 flow. Backend, Apps Script,
`session.js` и Virtual Portfolio в рамках этапа 9 не изменяются.
